...
Acl.prototype.removeUserRoles = function(userId, roles, cb){
  contract(arguments).params('string|number','string|array','function').end();
...
Acl.prototype.userRoles = function(userId, cb){
  contract(arguments).params('string|number', 'function').end();
...
/**
   Removes the prefix from any resource
   @param  {String} prefix string to remove from the begining of any resource
 */
Acl.prototype.ignorePrefix = function(prefix) {
  contract(arguments)
    .params('string');
  ...
  if((arguments.length===2)&&_.isObject(roles)&&_.isFunction(resources)){
    ...
    resources.forEach(function(resource){
      ...
      if (self.removePrefix) {
        resource = resource.replace(self.removePrefix, '');
      }
//      self.backend.add(transaction, allowsBucket(resource), roles, permissions);
      roles.forEach(function(role){
        self.backend.add(transaction, allowsBucket(resource), role, permissions);
      });
    });
...
Acl.prototype.middleware = function(numPathComponents, userId, actions){
  ...
  return function(req, res, next){
    ...
    if (acl.removePrefix) {
        resource = resource.replace(acl.removePrefix, '');
    }
